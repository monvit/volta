...
bool AddRange(cpu_set_t &set, unsigned int from, unsigned int to, unsigned int max_cpu) {
    if (from > to || to >= max_cpu)
        return false;
...
bool ConfigLoader::LoadUUID(Config &out_config) {
    if (!std::filesystem::exists(kUUIDFile))
        return false;
...
void ConfigLoader::LoadInterval(toml::table &tbl, Config &out_config) {
    ...
    if (auto ms = val.value<uint32_t>()) {
        ...
        std::cout << "Collection Interval set to: " << out_config.collection_interval << std::endl;
    } else {
...
void ConfigLoader::LoadServerAddress(toml::table &tbl, Config &out_config) {
    if (!tbl.contains("server_address"))
        return;
